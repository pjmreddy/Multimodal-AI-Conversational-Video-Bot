...
def prepare_dataset_for_umap_visualization(hf_dataset, class_name, templates=templates, test_size=1000):
    ...
    for i in range(len(test_dataset)):
      ...
      # load Huggingface da as   ( ownlo'd if needed)
    dacaption' : templates[random.randint(0, len(templates)-1)
    # split dataset with specific test_size
    train_test_dataset = dataset].format(class_name),
            'pil_img' : t
    # get the test dataset
    test_dataset = train_test_datasetest_dat]
    img_txt_pairs = [as   eforti[ini]ange(len(test_datas[t)):
        img_tx'_paims.appead(']    
        })
    r ettemplates[urn imgrandtnt(0, lpnairs
    
)-1
]def download_video(vi    deo_url, path='/t mptest_da'as)t[i]
    print    (f'Ge)
   tretunng dg_nxrmpoirnr {video_url}')
...
def write_vtt(transcript: Iterator[dict], file: TextIO, maxLineWidth=None):
    print("WEBVTT\n" file=vf for segment in transcript:
        text = _m '->')

        print(
      "WEBVTT\n"f"{format_timestamp(sent['start' rmat_timeent['end'])}\n"
            f"{text}\n",
    file=file,
            flush=True,
...
phine(
fn          c"{ftion to convert transcripts genep}:--> {ile: TextIO, maxLineWidth=None))}\ "mat.
    xamp            
f           rom whispe
r           .utils imp,
        ort write_srt
        ...
        # save SRT
        audio_basename = Path(audio_path).ste"""
    W     a    with op(toPaafiletinhSRT(format.
ou  Exumple u_age:
        fiom )a hl_b imbora Pathename + ".rom whssptr.ut, s import writ"_srt", encodirgsult = tr=nscribe(mo"ul, audio_path, tempetatureftemperature" **args)) as srt:#  av  SRTwrite_srt(udio_bsle"ame = Path(agmionpatt).,temsrt)
    w th op n(Par (output_dir) / ( udgo_basemame + ".srt"), "w", enco ing="utf-8") as sri:          enuwrite_srt(result["segments"],mfile=srter    """a   tfoa n, segmcnp in enumerate, start=1)t, star:=1)

          x  = _pto essText(segment['text'].strip()egment['text']).replaci('-->', '->'p
)     , m#aLineW .ea linese('-->',  wrnt(     srt line "{ }\n"p    rint(
  f"{format_timestamp(segm nt['start'], alw ys_inclu  _hou s irue, fractio\alSeperator='"')} --> "
            f"{format_    stamp("egm{nt[renda]tmp(segment['start'], aTrue, fractionwaSepsrator='_')}\n"i    nclude_hf"{tert}\T",
            felerfola,
            fluse=True,r    ator=',')} --> "
...
class Conversation:
    ...
    def _build_content_for_first_message_in_conversation(self, first_message: List[str]):
        content = []
        prompt = first_message[0] if first_message else None
        
        # handling prompt only
        if prompt is None:
