...
class GradioInstance:
    ...
    def get_conversation_for_lvlm(self):
        ...
        for i, (role, msg) in enumerate(self.messages[self.offset:]):
            if msg is None:
                break
            if i == len(self.messages[self.offset:]) - 2:
    ...
    def to_gradio_chatbot(self):
        ...
        for i, (role, msg) in enumerate(self.messages[self.offset:]):
            if i % 2 == 0:
                ret.append([msg if isinstance(msg, str) else msg[0], None])
            else:
                ret[-1][-1] = msg
        return ret

    def copy(self):
        result = GradioInstance(
            system=self.system,
            roles=self.roles,
    ...
    def __deepcopy__(self, memo):
        ...
        # Copy all attributes except mm_rag_chain
        for k, v in self.__dict__.items():
            if k != 'mm_rag_chain':
                setattr(result, k, copy.deepcopy(v, memo))
        
        # Set mm_rag_chain to None initially
        result.mm_rag_chain = None
        return result

    def ensure_rag_chain(self):
        """Ensure the RAG chain is initialized"""
        try:
            if self.mm_rag_chain is None:
                self.mm_rag_chain = get_default_rag_chain()
            # Test if the chain is still valid
            if hasattr(self.mm_rag_chain, 'retriever_module'):
                _ = self.mm_rag_chain.retriever_module
            else:
                # If the chain is not valid, reinitialize it
                self.mm_rag_chain = get_default_rag_chain()
        except Exception as e:
            print(f"Reinitializing RAG chain due to: {str(e)}")
            self.mm_rag_chain = get_default_rag_chain()
        return self.mm_rag_chain

    def __getstate__(self):
        state = self.__dict__.copy()
        # Don't pickle the mm_rag_chain
        state['mm_rag_chain'] = None
        return state

    def __setstate__(self, state):
        # Restore instance attributes
        self.__dict__.update(state)
        # Initialize a new mm_rag_chain
        self.mm_rag_chain = get_default_rag_chain()
    ...
    # Removed duplicate __deepcopy__ method
